...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        ##
        ##checks the state of the machine then updates lights based on
        ##state, setpoint, and current temp
    ...
    def setupSerialOutput(self):
        ...
        output = f"{self.current_state.id}, Set:{self.setPoint}, Current Temp:{self.getFahrenheit():0.1f}F"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ##
            ##adds the current date and time to LCD screen
            lcd_line_1 = current_time.strftime('%b %d  %H:%M:%S\n')
    
            ## Setup Display Line 2
            if(altCounter < 6):
                ##
                ##Puts current temp on LCD screen
                lcd_line_2 = f"T:{self.getFahrenheit():0.1f}F"
    
                altCounter = altCounter + 1
            else:
                ##
                ##Puts current state and setpoint on LCD screen
                lcd_line_2 = f"{self.current_state.id} Set:{self.setPoint}"
