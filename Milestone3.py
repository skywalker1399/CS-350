...
class CWMachine(StateMachine):
    ...
    def toggleMessage(self):
        ...
        #changes message based on what message is currently active
            ...
            for word in wordList:
                ...
                for char in word:
                    ...
                    for x in morse:
                        ...
                        ##if symbol is .
                        if x == ".":
                            #changes the state from off to dot
                            self.doDot()
                            #waits
                            #sleep(.5)
                            #changes state back to off
                            self.doDot()

                        ##if symbol is -
                        if x == "-":
                            #changes state from off to dash
                            self.doDash()
                            #waits
                            #sleep(1.5)
                            #changes state back to off
                        ...
                        ##MAYBE!!!! MIGHT NEED ERASED!!!!
                        #self.ddDDP()



                        ##checks if the total number of characters in a letter was reached
                        if morseCounter < lenMorse:
                            #changes state from off to dotdashpause
                            self.doDDP()
                            #waits
                            #sleep(.250)
                            #changes state back to off
                            self.doDDP()
                            #increases the morseCounter
                            morseCounter += 1
                    ...
                    #checks if the total number of letters in a word was reached
                    if wordCounter < lenWord:
                        #changes state form off to letterpause
                        self.doLP()
                        #waits
                        #sleep(.750)
                        #changes state back to off
                ...
                #checks if the total number of words was reached
                if wordsCounter < wordCounter:
                    #changes state from off to wordpause
                    self.doWP()
                    #waits
                    #sleep(3)
                    #changes state back to off
                    self.doWP()
                    #increase wordscounter
                    wordsCounter += 1
...
## takes green button pressed input to change word
